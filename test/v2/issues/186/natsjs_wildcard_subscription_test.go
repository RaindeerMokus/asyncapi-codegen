...
func (suite *Suite) TestIssue186Star() {
	...
	// Register consumer
	err := suite.app.SubscribeV2Issue186Star(context.Background(),
		func(ctx context.Context, msg V2Issue186StarMessage) error {
			suite.Require().Equal(payload, msg.Payload)
			wg.Done()
			return nil
		})
	suite.Require().NoError(err)
	defer suite.app.UnsubscribeV2Issue186Star(context.Background())
...
func (suite *Suite) TestIssue186Angle() {
	...
	// Register consumer
	err := suite.app.SubscribeV2Issue186Angle(context.Background(),
		func(ctx context.Context, msg V2Issue186AngleMessage) error {
			suite.Require().Equal(payload, msg.Payload)
			wg.Done()
			return nil
		})
	suite.Require().NoError(err)
	defer suite.app.UnsubscribeV2Issue186Angle(context.Background())
